...
parser.add_argument('-num_iters', type=int, default=201, help='Number of iterations')
...
class Burgers:
    def __init__(self, viscosity=0.01):
        ...
        self.max_steps = params['num_iters']
    ...
    def simulate(self):
        for i in range(self.max_steps):
            ...
            v1 = diffuse.explicit(self.velocities[-1], self.viscosity, params['dt']) + self.forcing * params['dt']
            v2 = advect.semi_lagrangian(v1, v1, params['dt'])

            self.velocities.append(v2)

            self.save_simulation_data(i)
            # # Save data to the scene every 100 steps
            # if i % 100 == 0:
            #     self.save_simulation_data(i)
            #     print(f'Step {i} done')
